<template>
  <div class="navbar h-[4.8rem] mt-0 bg-rsmp-sec">
        ...
        <ul class="menu menu-horizontal px-1">
          <li class="mr-[1rem]">
            <button
              ...
              class="map-btn"
              :class="view == 'map' ? 'bg-rsmp-active-btn' : ''"
            >
              Map View
            </button>
  ...
  { name: "chart", title: "Dashboard" }, -->
          ...
          <li>
            <button
              @click.prevent="changeView('ptins')"
              class="map-btn"
              :class="view == 'ptins' ? 'bg-rsmp-active-btn' : ''"
            >
              Support Summary
            </button>
          </li>
...
.map-btn {
  ...
  /* background: rgba(0, 204, 190, 0.25); */
  border-radius: 8px;
  color: aliceblue;
  font-size: medium;
